	...
	>
		<div class="px-6 pt-4 pb-5">
			<div class="flex items-center justify-between">
				<a href="/">
					<span class="font-bold text-md text-black">Mortise &AMP; Code</span>
				</a>
				<div class="-mr-2">
					<PopoverButton
			...
			<ul class="font-bold text-xls text-neutral-900 py-5">
				{#each navLinks as link}
					<a
						href={link.href}
						class="hover:text-goldbrush-500 bg-slate-100"
						on:click={() => {
							close(null);
						}}
					>
				...
				<ul>
					{#each socialLinks as link}
						<a href={link.href} rel="me" class="w-auto h-auto inline-block p-3" target="_blank">
							<svelte:component
								this={link.icon}
								class="text-neutral-800 hover:text-goldbrush-500"
							/>
						</a>
					{/each}
				</ul>
